...
#
# "#2F4A4A","#588B9B", "#A9707F","#D1A6B2", "#CEB59B", "#C7C7C7"

#png(filename = "4-Figures/eb-analysis-summary.png", width = 8, height = 5, units = "in", res = 300)
ggplot(pred.mod7.b) +
  geom_ribbon(aes(fill = group, x = x, ymin = conf.low, ymax = conf.high), alpha = 0.2)+
...
#png(filename = "4-Figures/eb-analysis-summary_inlet.png", width = 8, height = 5, units = "in", res = 300)
ggplot(pred.mod7.a) +
  #geom_ribbon(aes(fill = group, x = x, ymin = conf.low, ymax = conf.high), alpha = 0.2)+
 geom_line(aes(color = group, x = x, y = conf.low), lty = 2)+
  geom_line(aes(color = group, x = x, y = conf.high), lty = 2)+
  geom_point(data=ebullition_24, 
             aes(x=inlet_dist, y = ch4_eb_g_m2_hr_24, color=Sampling),
             size = 2.5, shape = 1, stroke = 1)+
   geom_line(aes(x=x, y=predicted, color = group), lwd = 2)+
  scale_color_manual(values = c("#DCE319FF","#CD984B","#55C667FF", "#143C1B", "#440154FF", "#2D708EFF"),
                     labels = c("Catfish 09/06/22", "Catfish 09/18/22", "Deans 08/16/22", "Deans 08/30/22", "Pick 09/13/22", "Sister 08/22/22"))+
  scale_fill_manual(values = c("#DCE319FF","#CD984B","#55C667FF", "#143C1B", "#440154FF", "#2D708EFF"),
                     labels = c("Catfish 09/06/22", "Catfish 09/18/22", "Deans 08/16/22", "Deans 08/30/22", "Pick 09/13/22", "Sister 08/22/22"))+
...
#  geom_line(aes(color = group, x = x, y = conf.high), lty = 2)+
  geom_point(data=ebullition_24, 
             aes(x=inlet_dist, y = ch4_eb_g_m2_hr_24),
             size = 2.5, shape = 1, stroke = 1)+
   geom_line(aes(x=x, y=predicted), lwd = 2)+
  #scale_color_manual(values = c("#DCE319FF","#CD984B","#55C667FF", "#143C1B", "#440154FF", "#2D708EFF"),
  #                   labels = c("Catfish 09/06/22", "Catfish 09/18/22", "Deans 08/16/22", "Deans 08/30/22", "Pick 09/13/22", "Sister 08/22/22"))+
  #scale_fill_manual(values = c("#DCE319FF","#CD984B","#55C667FF", "#143C1B", "#440154FF", "#2D708EFF"),
  #                   labels = c("Catfish 09/06/22", "Catfish 09/18/22", "Deans 08/16/22", "Deans 08/30/22", "Pick 09/13/22", "Sister 08/22/22"))+
