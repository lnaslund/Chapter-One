...
# 
# write.csv(deans_16_eb_results_summarized, "5-Results/1-kriged-estimates/deans_16_eb_results_summarized.csv")
# 
# deans_16_ch4_diff <- read.csv("5-Results/1-kriged-estimates/deans_16_ch4_results_summarized.csv") %>% mutate(pathway = "diffusion", gas = "CH4", period = as.character(period), avg_time = 3, sampling = "Deans1")
# 
...
Catfish 9/18/22
```{r}
# catfish2_conc <- read.csv("ebullition_concentrations.csv")%>% 
#   mutate(corr_ch4 = case_when(
#   Sample.Vol == 0.1 ~ Undiluted.CH4.conc.ppm * 0.473176497, 
#   Sample.Vol == 0.2 ~ Undiluted.CH4.conc.ppm, 
#   Sample.Vol == 1 ~ Undiluted.CH4.conc.ppm)) %>% 
...
Pick 9/13/22
```{r}
# pick1_conc <- read.csv("ebullition_concentrations.csv")%>% 
#   mutate(corr_ch4 = case_when(
#   Sample.Vol == 0.1 ~ Undiluted.CH4.conc.ppm * 0.473176497, 
#   Sample.Vol == 0.2 ~ Undiluted.CH4.conc.ppm, 
#   Sample.Vol == 1 ~ Undiluted.CH4.conc.ppm)) %>% 
...
#                 mutate(ch4_eb_g_m2_hr = gas_vol_corrected * (1/area) * (1/time_hr)* (1/10^6) * ch4_conc * (1/10^3) * 16.04 * (1/0.0821)* (1/293.15)), by = "location")
# 
# pick_13_eb_results <- read.csv("pick_13_eb_results.csv")
...
#                           site == "Picks" ~ st_area(pick) %>% as.numeric(),
