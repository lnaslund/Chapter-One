...
ws <- read.csv("2-Clean Data/windspeeds.csv") %>% mutate(date = ymd(date))%>% 
...
lux <- read.csv("2-Clean Data/lux.csv") %>% mutate(Date_Time = ymd_hms(Date_Time, tz = "America/New_York")) %>% 
  ...
  distinct()%>% 
   mutate(sampling = case_when(
    (Date_Time > ymd_hms("2022-09-06 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-09-07 08:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish 9/6 - 9/7",
    (Date_Time > ymd_hms("2022-09-18 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-09-19 08:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish 9/18 - 9/19",
    (Date_Time > ymd_hms("2022-08-16 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-08-17 08:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans 8/16 - 8/17",
    (Date_Time > ymd_hms("2022-08-30 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-08-31 08:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans 8/30 - 8/31",
    (Date_Time > ymd_hms("2022-09-13 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-09-14 08:00:00", tz = "America/New_York") & site == "Blue Herron") ~ "Blue Herron 9/13 - 9/14",
    (Date_Time > ymd_hms("2022-08-22 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-08-23 08:00:00", tz = "America/New_York") & site == "Sister") ~ "Sister 8/22 - 8/23",
    TRUE ~ NA_character_
  ))

time_join <- join_by(site, closest(x$start_time >= y$Date_Time))

diffusion_env <- diffusion %>% 
  mutate(site = case_when(
    site == "Catfish" ~ "Catfish",
    site == "Deans" ~ "Deans", 
    site == "Pick" ~ "Blue Herron",
...
diffusion_env <- diffusion_env %>% 
  left_join(ws %>% select(-date, -site), by = c("sampling", "location", "trip")) %>% 
  drop_na()

time_plot_aes <- function(x, date_time_vec){
  return(x +theme_bw()+
...
ysi <- read.csv("2-Clean Data/ysi_all.csv") %>% mutate(sampling = case_when(
  sampling == "Catfish1" ~ "Catfish 9/6 - 9/7",
  sampling == "Catfish2" ~ "Catfish 9/18 - 9/19",
  sampling == "Deans1" ~ "Deans 8/16 - 8/17",
  sampling == "Deans2" ~ "Deans 8/30 - 8/31",
  sampling == "Pick1" ~ "Blue Herron 9/13 - 9/14",
  sampling == "Sister31" ~ "Sister 8/22 - 8/23"
...
diffusion_env <- left_join(diffusion_env, ysi %>% filter(name == "YSI Bottom DO mg/L" | name == "YSI Top DO mg/L") %>% select(sampling, location, trip, name, do) %>%  pivot_wider(id_cols = c(sampling, location, trip), names_from = name, values_from = do) %>% group_by(sampling, trip) %>% summarize(mean_top_do = mean(`YSI Top DO mg/L`, na.rm = T), mean_bottom_do = mean(`YSI Bottom DO mg/L`, na.rm = T)) , by = c("sampling", "trip")) %>% mutate(start_date = case_when(
  sampling == "Catfish 9/6 - 9/7" ~ ymd("2022-09-06"),
  sampling == "Catfish 9/18 - 9/19" ~ ymd("2022-09-18"),
  sampling == "Deans 8/16 - 8/17" ~ ymd("2022-08-16"),
  sampling == "Deans 8/30 - 8/31" ~ ymd("2022-08-30"),
  sampling == "Blue Herron 9/13 - 9/14" ~ ymd("2022-09-13"),
  sampling == "Sister 8/22 - 8/23" ~ ymd("2022-08-23")
))
```
TODO: separate lux, temp, bottom, top do, and ws model and then mutliple of those
