...
do <- do %>%
  ...
  mutate(sampling = case_when(
    (Date_Time > ymd_hms("2022-09-06 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-09-07 08:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish \n2022/09/06 - 2022/09/07",
    (Date_Time > ymd_hms("2022-09-18 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-09-19 08:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish \n2022/09/18 - 2022/09/19",
    (Date_Time > ymd_hms("2022-08-16 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-08-17 08:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans \n2022/08/16 - 2022/08/17",
    (Date_Time > ymd_hms("2022-08-30 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-08-31 08:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans \n2022/08/30 - 2022/08/31",
    (Date_Time > ymd_hms("2022-09-13 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-09-14 08:00:00", tz = "America/New_York") & site == "Blue Herron") ~ "Blue Herron \n2022/09/13 - 2022/09/14",
    (Date_Time > ymd_hms("2022-08-22 08:00:00", tz = "America/New_York") & Date_Time < ymd_hms("2022-08-23 08:00:00", tz = "America/New_York") & site == "Sister") ~ "Sister \n2022/08/22 - 2022/08/23",
    TRUE ~ NA_character_
  ))
```
...
anno <- data.frame(
  x1 = c(ymd_hms("2022-09-06 08:00:00", tz = "America/New_York"), ymd_hms("2022-09-18 08:00:00", tz = "America/New_York"), ymd_hms("2022-08-16 08:00:00", tz = "America/New_York"), ymd_hms("2022-08-30 08:00:00", tz = "America/New_York"), 
         ymd_hms("2022-09-13 08:00:00", tz = "America/New_York"), ymd_hms("2022-08-22 08:00:00", tz = "America/New_York") ),
                   x2 = c(ymd_hms("2022-09-07 08:00:00", tz = "America/New_York"), ymd_hms("2022-09-19 08:00:00", tz = "America/New_York"), ymd_hms("2022-08-17 08:00:00", tz = "America/New_York"), 
                          ymd_hms("2022-08-31 08:00:00", tz = "America/New_York"), ymd_hms("2022-09-14 08:00:00", tz = "America/New_York"), ymd_hms("2022-08-23 08:00:00", tz = "America/New_York")),
                   y1 = c(rep(0,6)),
                   y2 = c(rep(11,6)),
...
do %>% filter(location == "Top") %>% mutate(site = if_else(site == "Pick", "Blue Herron", site)) %>% 
  filter(is.na(sampling)==F) %>% 
  ggplot(aes(Date_Time, DO_mgL, group = sampling)) +
  geom_line()+
  theme_bw()+
  ylab("DO (mg/L)") + 
  theme(axis.title.x = element_blank(), axis.text = element_text(size = 14, color = "black"), 
...
all_temps <- all_temps %>% 
  ...
  mutate(sampling = case_when(
    (date > ymd_hms("2022-09-06 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-09-07 08:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish \n2022/09/06 - 2022/09/07",
    (date > ymd_hms("2022-09-18 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-09-19 08:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish \n2022/09/18 - 2022/09/19",
    (date > ymd_hms("2022-08-16 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-08-17 08:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans \n2022/08/16 - 2022/08/17",
    (date > ymd_hms("2022-08-30 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-08-31 08:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans \n2022/08/30 - 2022/08/31",
    (date > ymd_hms("2022-09-13 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-09-14 08:00:00", tz = "America/New_York") & site == "Blue Herron") ~ "Blue Herron \n2022/09/13 - 2022/09/14",
    (date > ymd_hms("2022-08-22 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-08-23 08:00:00", tz = "America/New_York") & site == "Sister") ~ "Sister \n2022/08/22 - 2022/08/23",
    TRUE ~ NA_character_
  ))

ggplot(all_temps %>% filter(is.na(sampling) == F, depth == 0.25), aes(date, temp, group = sampling)) + 
  geom_line() + 
  facet_wrap(~sampling, scales = "free_x")+
   scale_x_datetime(date_labels = "%H:%M")+
  theme_bw()+
...
top_temp <- all_temps %>% 
  mutate(sampling = case_when(
    (date > ymd_hms("2022-09-06 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-09-07 12:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish 9/6 - 9/7",
    (date > ymd_hms("2022-09-18 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-09-19 12:00:00", tz = "America/New_York") & site == "Catfish") ~ "Catfish 9/18 - 9/19",
    (date > ymd_hms("2022-08-16 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-08-17 12:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans 8/16 - 8/17",
    (date > ymd_hms("2022-08-30 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-08-31 12:00:00", tz = "America/New_York") & site == "Deans") ~ "Deans 8/30 - 8/31",
    (date > ymd_hms("2022-09-13 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-09-14 12:00:00", tz = "America/New_York") & site == "Blue Herron") ~ "Blue Herron 9/13 - 9/14",
    (date > ymd_hms("2022-08-22 08:00:00", tz = "America/New_York") & date < ymd_hms("2022-08-23 12:00:00", tz = "America/New_York") & site == "Sister") ~ "Sister 8/22 - 8/23",
    TRUE ~ NA_character_
  )) %>% 
